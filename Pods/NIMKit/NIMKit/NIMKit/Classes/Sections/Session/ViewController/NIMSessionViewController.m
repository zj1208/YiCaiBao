...
- (void)setupNav
{
    ...
    leftBarView.badgeView.frame = CGRectMake(-16, 0, 40, 40);
    leftBarView.frame = CGRectZero;
    UIBarButtonItem *leftItem = [[UIBarButtonItem alloc] initWithCustomView:leftBarView];
    
    UIBarButtonItem *barButtonItem = [[UIBarButtonItem alloc] initWithImage:[[UIImage imageNamed:@"back_imageTitle"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] style:UIBarButtonItemStyleDone target:self action:@selector(customBackItemClicked)];
    self.navigationItem.leftBarButtonItems = @[barButtonItem,leftItem];
}
...
- (void)changeLeftBarBadge:(NSInteger)unreadCount
{
    ...
    NIMCustomLeftBarView *leftBarView = (NIMCustomLeftBarView *)barButtonItem.customView;

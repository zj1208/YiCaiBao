...
@interface SoldOutProductController ()<ZXEmptyViewControllerDelegate>
...
@implementation SoldOutProductController
...
- (void)setData{
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(updateData:) name:Noti_ProductManager_updateSoldouting object:nil];
...
{
    WS(weakSelf);
    [ProductMdoleAPI getMyProductListWithType:MyProductType_soldoution onlyMain:NO direction:self.direction pageNo:1 pageSize:@(10) success:^(id data,PageModel *pageModel) {
        
        [weakSelf.dataMArray removeAllObjects];
        [weakSelf.dataMArray addObjectsFromArray:data];
        [_emptyViewController addEmptyViewInController:weakSelf hasLocalData:_dataMArray.count error:nil emptyImage:[UIImage imageNamed:@"产品公开为空"] emptyTitle:@"暂无下架中产品~" updateBtnHide:YES];
...
{
    WS(weakSelf);
    [ProductMdoleAPI getMyProductListWithType:MyProductType_soldoution onlyMain:NO direction:self.direction pageNo:_pageNo+1 pageSize:@(10) success:^(id data,PageModel *pageModel) {
...
{
    SoldOutProductCell *cell = (SoldOutProductCell *)[tableView dequeueReusableCellWithIdentifier:@"Cell" forIndexPath:indexPath];
...
- (void)editProduct:(UIButton *)sender
{
    ...
    NSMutableArray *arrayM = [NSMutableArray arrayWithObjects:@"上架",@"删除",@"转为私密", nil];
...
//        }
//        //展示氛围图
//        [_emptyViewController addEmptyViewInController:self hasLocalData:NO error:nil emptyImage:[UIImage imageNamed:@"产品公开为空"] emptyTitle:@"暂无下架中产品~" updateBtnHide:YES];
//    }
//    else
//    {
//
//        [MBProgressHUD zx_showSuccess:@"成功上架产品" toView:nil];

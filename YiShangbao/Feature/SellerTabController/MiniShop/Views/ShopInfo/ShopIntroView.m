...
- (id)init {
    ...
    [self.textContent addPlaceHolder:@"主要介绍产品特色，资源优势，公司背景，批发事项哦～"];
...
//        make.top.equalTo(self.viewBg.mas_top).offset(14);
        if (@available(iOS 11.0, *)) {
            make.top.equalTo(self.viewBg.mas_safeAreaLayoutGuideTop).offset(10);
        } else {
            make.top.equalTo(self.viewBg.mas_top).offset(10);
        }
        make.width.equalTo(@(SCREEN_WIDTH-24));
        make.bottom.equalTo(self.viewBg.mas_bottom).offset(-26);
 ...
 replacementText:(NSString *)text
{
    ...
    if (selectedRange && pos) {
        ...
        if (offsetRange.location < MAX_LIMIT_NUMSI) {
            return YES;
        }
        else
        {
    ...
    NSInteger caninputlen = MAX_LIMIT_NUMSI - comcatstr.length;
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    if (existTextNum > MAX_LIMIT_NUMSI)
    {
        //截取到最大位置的字符(由于超出截部分在should时被处理了所在这里这了提高效率不再判断)
        NSString *s = [nsTextContent substringToIndex:MAX_LIMIT_NUMSI];
        
        [textView setText:s];
    }
    
    //不让显示负数
    self.lblChart.text = [NSString stringWithFormat:@"还可输入%ld字",MAX(0,MAX_LIMIT_NUMSI - existTextNum)];
}
...
#pragma mark - 初始化
-(void)initUI:(id)data{
    NSString *content = [data objectForKey:@"outline"];
    if (content.length) {
        self.textContent.text = content;
        self.lblChart.text = [NSString stringWithFormat:@"还可输入%ld字",((long)MAX_LIMIT_NUMSI-content.length)];
        [self.textContent hiddenplaceHolderTextView];
    }
}

...
@implementation ChangePhoneStepTwoView
...
- (id)init {
    ...
    [self.scroll_bg addSubview:self.label_notice];
    self.label_notice.textColor = WYUISTYLE.colorBTgrey;
    self.label_notice.font = [UIFont systemFontOfSize:13];
    self.label_notice.textAlignment = NSTextAlignmentCenter; 
    self.label_notice.text = @"更换成功后，即可使用新的手机号登录";
    ...
    [self.btn_confirm setTitle:@"确认更换" forState:UIControlStateNormal];
    ...
    [self.viewbg mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.label_notice.mas_bottom);
        make.left.equalTo(@0);
        make.width.equalTo(@(SCREEN_WIDTH));

...
//
//  简介：webView中所有设计的内容（不包括滚动列表）不要被屏幕圆角、上方传感器区域和下方home键指示器区域遮挡，要在安全区域safeArea布局content；但是webView中底部有可能会有按钮，除非前端做适配；webView执行刷新数据，刷新UI必须在屏幕栈区顶部展示的时候渲染才有效，涉及到执行js刷新，reloadData等；

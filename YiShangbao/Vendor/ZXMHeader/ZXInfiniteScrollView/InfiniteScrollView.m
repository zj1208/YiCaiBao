...
//    这里早点可以调用scrollrectto方法，就不会出现测试太快导致出现空白面来不及现实的问题了
    if (scrollView.dragging)
    {
        ...
        int currentPage = floor((scrollView.contentOffset.x - 0) / pagewidth);
        
        //    这里的scrollView offest是减速停止的时候； 这里的currentPage 和 scrollViewDidScrollView得到的一样；
        if (currentPage==-1)
        {
            [scrollView scrollRectToVisible:CGRectMake(CGRectGetWidth(scrollView.frame) * self.itmesArray.count,0,CGRectGetWidth(scrollView.frame),CGRectGetHeight(scrollView.frame)) animated:NO]; // 第一页的时候 变为最后1页
        }

...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (scrollView.tag == ScrollViewTagValue_MONTHS)
    {
        if (comps.month < index+1&&_selectedYear ==comps.year)
        {
            return;
        }
    }
    ...
    {
        if (comps.month < _selectedMonth &&_selectedYear ==comps.year)
        {
            [self setScrollView:self.scrollViewMonths atIndex:0 animated:YES];
            return;
        }
    }
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate {
    ...
    if (scrollView.tag == ScrollViewTagValue_MONTHS)
    {
        if (comps.month < index+1 &&_selectedYear ==comps.year)
        {
            [self setScrollView:scrollView atIndex:comps.month-1 animated:YES];
            return;
        }
    }
    else if (scrollView.tag == ScrollViewTagValue_YEARS)

...
- (void)_sjSetupView {
    ...
    _areaView.selectTimeTitle1 = @"10秒";
//    _areaView.selectTimeTitle2 = @"3分钟";
    
    __weak typeof(self) _self = self;
    _areaView.exeSelectTime1Block = ^{
        _self.areaView.minDuration = 0.1;
        _self.areaView.maxDuration = 10;
        // 设置完时间 更新一下标记位置
...
- (void)addDeviceOrientation{
    //    CMAccelerometerHandler *motionHandler;
    ...
    [self.motionManger startAccelerometerUpdatesToQueue:[NSOperationQueue mainQueue] withHandler:^(CMAccelerometerData *accelerometerData, NSError *error) {
        ...
        if (device_angle > M_PI)
            device_angle -= 2 * M_PI;
        //        if ((zz < -.60f) || (zz > .60f)) {
        //            if ( UIDeviceOrientationIsLandscape(UIDeviceOrientationPortrait) )
        ...
        if (orientation != UIDeviceOrientationPortrait) {
            dispatch_async(dispatch_get_main_queue(), ^{
                selfBlock.recordingOrientation = orientation;
            });
        }
    }];
}

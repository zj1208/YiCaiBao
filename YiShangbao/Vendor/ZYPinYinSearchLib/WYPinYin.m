...
static char WY_firstLetterArray[HANZI_COUNT] =
...
char pinyinFirstLetter(unsigned short hanZi)
{
    ...
    if (index >= 0 && index <= HANZI_COUNT){
        return WY_firstLetterArray[index];
...
@implementation NSString (WYFirstLetter)

- (NSString *)firstLetter
{
    return [WYFirstLetter firstLetter:self];
}

- (NSString *)firstLetters
{
    return [WYFirstLetter firstLetters:self];
}

@end

@implementation NSArray (WYFirstLetterArray)
...
@implementation WYFirstLetter

+ (NSString *)firstLetter:(NSString *)chineseString
{
    return [WYFirstLetter firstLetterUsingSeperate:@"" chineseString:chineseString isFirst:YES isReplace:YES];
}

+ (NSString *)firstLetters:(NSString *)chineseString
{
    return [WYFirstLetter firstLetterUsingSeperate:@"" chineseString:chineseString isFirst:NO isReplace:YES];
}

+ (NSString *)firstLetterUsingSeperate:(NSString *)seperate chineseString:(NSString *)chineseString isFirst:(BOOL)isFirst isReplace:(BOOL)yes
{
    ...
    [chineseString enumerateCharaterUsingBlock:^(unsigned short letter, BOOL *stop) {
        ...
        *stop = isFirst;
        ...
        if (index >= 0 && index <= HANZI_COUNT){ //汉字
            
            if ([firstLetters length]){
                firstLetters = [firstLetters stringByAppendingFormat:@"%@%c",seperate, WY_firstLetterArray[index]];
            }else {
                firstLetters = [firstLetters stringByAppendingFormat:@"%@%c",seperate, WY_firstLetterArray[index]];
            }
            ...
            //如果是字母或其它符号，返回 #
            if ([firstLetters length]){
                firstLetters = [firstLetters stringByAppendingFormat:@"%c",yes ? '#' : letter];
            }else {
                firstLetters = [firstLetters stringByAppendingFormat:@"%c", yes ? '#' : letter];
            }
        }
